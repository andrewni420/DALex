...
from push4.gp.selection import Lexicase, PLexicase
...
def run(problem: Problem, downsample_rate=1., alpha=1.):
    ...
    # The parent selector.
    selector = PLexicase(alpha=alpha)
...
if __name__ == "__main__":
    ...
    out_dir = "results/ds_{}/{}-lexiprob-{}-mul-1000cases".format(
        downsample_rate, problem_name, alpha)
    os.makedirs(out_dir, exist_ok=True)

    out_file = out_dir+'/exp_{}.json'.format(exp_id)
    if not os.path.exists(out_file):
        test_err, logs, fitnesses = run(problem, downsample_rate, alpha)
        results = logs
