...
class DataManagement:
    def __init__(self, trainFile, testFile, infoList = None):
        ...
        if cons.crossValidation:
            ...
            n=len(self.dataFormatted)
            indices = rng.choice(n, size=int(round(0.6*n)),replace=False)
            rng.shuffle(indices)
            self.trainFormatted = [self.dataFormatted[i] for i in indices[:int(round(2*len(indices)/3))]]
            self.testFormatted = [self.dataFormatted[i] for i in indices[int(round(2*len(indices)/3)):]]


            # self.trainFormatted = self.dataFormatted[:int(round(0.7*len(self.dataFormatted)))]
            # self.testFormatted = self.dataFormatted[int(round(0.7*len(self.dataFormatted))):]
        else:
