...
class ClassifierSet:
    ...
    def runGA(self, exploreIter, state, phenotype): 
        ...
        if cons.selectionMethod == "roulette": 
            ...
            clP1 = selectList[0]
            clP2 = selectList[1]
        elif cons.selectionMethod == "dalex":
            selectList = self.selectClassifierDALex(exploreIter)
            clP1 = selectList[0]
            clP2 = selectList[1]
        else:
    ...
    def selectClassifierLexBatch(self, exploreIter):
        """  Selects parents using batch-lexicase selection. """
        import numpy as np
        import random
        traindata = cons.env.formatData.trainFormatted
        if exploreIter < len(traindata):
        ...
        for k in range(0,2):
            ...
            # print(f"CORRECT SET {classifierList}")
            random.shuffle(data)
            batchsize = 100
    ...
    def tournamentAccuracy(self, exploreIter):
        """  Selects parents using batch-lexicase selection. """
        """  Selects parents using tournament selection according to the fitness of the classifiers. """
        selectList = [None, None]
        currentCount = 0 
        ...
        fitnesses = []
        for j in setList:
            correctCount=  len(self.popSet[j].correctList)
            matchCount= len(self.popSet[j].matchList)
            f = np.power(correctCount/matchCount,5)
            fitnesses.append(f)

        while currentCount < 2:
            tSize = int(len(setList)*cons.theta_sel)
            posList = random.sample(range(len(setList)),tSize) 

            bestF = 0
            bestC = self.correctSet[0]
            for j in posList:
                if fitnesses[j] > bestF:
                    bestF = self.popSet[setList[j]].fitness
                    bestC = setList[j]
    ...
    def selectClassifierDALex(self, exploreIter, std=50):
        selectList = [None, None]
        currentCount = 0 
        ...
        #Calculate the number of correct classifications and the number of matches for each rule
        corrects = np.zeros((len(setList),n))
        matches = np.zeros((len(setList),n))

        t = time.time()

        for _i,j in enumerate(setList):
            for k in self.popSet[j].correctList:
                corrects[_i,k]=1.
            for k in self.popSet[j].matchList:
                matches[_i,k]=1.

        #Standardize correctness scores. This helps one DALex particularity pressure setting work well across multiple problems
        correct_std = np.std(corrects,axis=0)
        correct_std = np.where(correct_std==0,1.,correct_std)
        corrects=(corrects-np.mean(corrects,axis=0))/correct_std
        
        #compute importance scores and test case weights
        scores = rng.standard_normal(size=[2,n])*std
        weights = softmax(scores,axis=1)

        #Since we're only concerned with classifier accuracy, divide correctness score by the number of matches to get the accuracy score
        fitnesses = np.matmul(corrects,np.transpose(weights))/np.matmul(matches,np.transpose(weights))

        #Select individuals
        indices = np.argmax(rng.random(size=fitnesses.shape)*(fitnesses==np.amax(fitnesses, axis=0,keepdims=True)),axis=0)
        selectList = [self.popSet[setList[i]] for i in indices]
    ...
    def addClassifierToPopulation(self, cl, covering):
        ...
        if oldCl != None: #found identical classifier
            ...
            self.popSet.append(cl)
            if cons.selectionMethod not in ['tournament', "roulette"]:
