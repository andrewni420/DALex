...
class Classifier:
    ...
    def error_vector(self, exploreIter,lexicase=False):
        data = cons.env.formatData.trainFormatted
        # maxlen = min(exploreIter,len(data))
        maxlen=len(data)

        matches = [0.]*maxlen
        errors = [0.]*maxlen
        for i in self.correctList:
            if i<maxlen:
                errors[i]=-1.
                matches[i]=1.
        for i in self.matchList:
            if i<maxlen:
                # errors[i]=2*errors[i]-1.
                matches[i]=1.
        if lexicase:
        ...
        # print(f"correct list {self.correctList} match list {self.matchList}")
        # print(f"error accuracy {pd(np.sum(np.array(errors)),np.sum(np.array(matches)))}")
        # print(f"true accuracy {self.accuracy}")
        # print(f"true fitness {self.fitness} (initial {cons.init_fit})")

        return np.array([errors]), np.array([matches])
    ...
    def report(self):
        ...
        # print(f"correct list {self.correctList} match list {self.matchList}")
        # print(f"error accuracy {pd(np.sum(np.array(errors)),np.sum(np.array(matches)))}")
        # print(f"synthetic accuracy {len(self.correctList)/len(self.matchList)}")
        # print(f"true accuracy {self.accuracy}")
        # print(f"true fitness {self.fitness} (initial {cons.init_fit})")
