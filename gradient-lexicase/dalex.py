...
select_criterion = nn.CrossEntropyLoss(reduction="none")
n_epoch = 200
...
def select(children, epoch):
    # validation_proportion = (epoch+1)/(n_epoch+1)
    # _,validationset = torch.utils.data.random_split(trainset, [1-validation_proportion,validation_proportion])
    validationloader = torch.utils.data.DataLoader(
        trainset, batch_size=10000, shuffle=True, num_workers=2)
    for net in children:
        net.eval()
    selection = list(range(pop_size))
    selected = np.random.randint(0, pop_size)

    losses = [[] for _  in selection]
    corrects = [[] for _ in selection]
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(validationloader):
            ...
            for child_idx in selection:
                outputs = children[child_idx](inputs)
                loss = select_criterion(outputs, targets)
                _, predicted = outputs.max(1)
                correct = 1-predicted.eq(targets).to(torch.float32)
                losses[child_idx].append(np.reshape(loss.cpu().detach().numpy(),[1,-1]))
                corrects[child_idx].append(np.reshape(correct.cpu().detach().numpy(),[1,-1]))

    losses = [np.reshape(arr,[1,-1]) for arr in losses]
    corrects = [np.reshape(arr,[1,-1]) for arr in corrects]
        
    t = time.time()
    if args.selector == "lexicase":
        selected,counts = lexicase(corrects)
        selected=selected[0]
        counts = counts[0]
    elif args.selector == "epsilon lexicase":
        selected,counts = lexicase(losses,epsilon=True)
        selected=selected[0]
        counts = counts[0]
    elif args.selector=="dalex":
        selected, counts = wlexicase(losses, std=args.std)
        selected=selected[0]
        counts = counts[0]
    elif args.selector=="dalex accuracy":
        selected, counts = wlexicase(corrects, std=args.std)
        selected=selected[0]
        counts = counts[0]
    elif args.selector=="first":
        num_cases = len(losses)
        rng=np.random.default_rng()
        case = rng.choice(num_cases,size=1).item()
        selected = np.argmin(np.array(losses)[:,0,case]).item()
        counts = 1
    elif args.selector == "tournament":
        rng=np.random.default_rng()
        fitness = np.mean(losses,axis=(1,2))
        tourn = rng.choice(len(fitness),size=2)
        selected = min(tourn,key=lambda i:fitness[i])
        counts = 50000
    elif args.selector == "tournament accuracy":
        rng=np.random.default_rng()
        fitness = np.mean(corrects,axis=(1,2))
        tourn = rng.choice(len(fitness),size=2)
        selected = min(tourn,key=lambda i:fitness[i])
        counts = 50000
    elif args.selector == "elite accuracy":
    ...
    global selection_times 
    selection_times.append(delta)
    global case_counts 
    case_counts.append(counts)
    
    # apply parent weights to all children
    print(f"Selected index {selected}")
    print(f"Cases considered {counts}")
    if isinstance(selected,list):
        weights = [deepcopy(children[i].state_dict()) for i in selected]
        for net,w in zip(children,weights):
...
for epoch in range(start_epoch, start_epoch+n_epoch*(pop_size+1)):
    ...
    t = time.time()
    children = select(children, epoch)
    select_times.append(time.time()-t)
    print(f"Selection runtime {select_times[-1]}")
