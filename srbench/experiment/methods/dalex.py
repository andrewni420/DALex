...
selection_logs = {"selection_runtime": []}
...
def _lexicase(parents, n=1):
    ...
    indices = lexicase(fitness,n=n,epsilon=True)

    return [(parents[i],i) for i in indices]


def _wlexicase(parents, n=1, std=1,distribution="normal"):
    if parents is None or len(parents)==0:
        return None
    popsize=len(parents)
    contenders=range(popsize)
    ...
    indices = wlexicase(fitness,n=n, std=std,distribution=distribution)
...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    programs = []
    n=n_programs*2
    selectors = {"weighted lexicase": lambda:_wlexicase(parents, n=n, std=std,distribution=distribution),
    ...
    selected_parents = selectors[selector]()
...
wlexicase_range = np.logspace(0,1.,num=9)
hyper_params = [{"std":[float(r)]} for r in wlexicase_range]
